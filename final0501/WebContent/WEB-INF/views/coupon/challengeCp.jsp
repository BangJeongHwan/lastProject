...
<div style="width: 70%;margin-left: 10%;">
		<div>
			<div class="regititle" style="width: 50%;">제목</div>
			<div class="regititle" style="width: 50%;">할인율</div>
		</div>
	<div>
		<c:choose>
			...
			<c:when test="${!empty registedCoupon}">
				<c:forEach items="${registedCoupon}" var="coupon" varStatus="vs">
					<div class="regicontent" style="width: 50%;">${coupon.title}</div>
					<div class="regicontent" style="width: 50%;">${coupon.discount}%</div>
				</c:forEach>
...
<table class="list_table" style="width:70%;margin-left: 10%;" id="tab1">
	...
	<tbody>	
		<c:if test="${empty couponlist}">
			<tr>
				<td colspan="3">작성된 쿠폰이 없습니다.</td>
			</tr>	
		</c:if>
	
		<c:forEach items="${couponlist}" var="coupon" varStatus="vs">
		<tr class="_hover_tr">
			<td><input type="checkbox" name="choice" id="_choice" value="${coupon.seq}"></td> 
			<td>${coupon.title}</td> 
...
<button style="position: fixed; margin-left: 72%; background: none; border: 3px solid inherit; border-radius: 5px; padding: 5px 15px; margin-top: 5%;" id="_regit">드응로크</button>
...
<script type="text/javascript">
	...
	$("#_regit").click(function () {
		var count = $("#_choice:checked").length

		if(count==3){
			
			$("#_choice:checked").each(function() {
	...
	function addSeq(test) {
		if(seq1 == null){
			...
			location.href="regChal.do?seq1=" + seq1 + "&seq2=" + seq2 + "&seq3=" + seq3 +"&sort=2";
		}
	}
