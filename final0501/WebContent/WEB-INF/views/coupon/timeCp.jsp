...
<div style="width: 70%;margin-left: 10%;">
		<div>
			...
			<div class="regititle" style="width: 30%;">시작 시간</div>
			<div class="regititle" style="width: 30%;">마감 시간</div>
	...
	<div>
		<c:choose>
			...
			<c:when test="${!empty registedCoupon}">
				...
				<div class="regicontent" style="width: 30%;">${registedCoupon.shour}시 ${registedCoupon.smin}분</div>
				<div class="regicontent" style="width: 30%;">${registedCoupon.timeremit}분</div>
...
<table class="list_table" style="width:70%;margin-left: 10%;" id="tab1">
	...
	<thead>
		<tr>
			<th>선택</th> <th>제목</th> <th>시작 시간</th> <th>마감 시간</th> <th>할인율</th>
		</tr>
	</thead>
	...
	<tbody>	
		...
		<c:forEach items="${couponlist}" var="coupon" varStatus="vs">
			<tr class="_hover_tr">
				...
				<td>${coupon.title}</td> 
				<td>${coupon.shour}시 ${coupon.smin}분</td>
				<td>${coupon.timeremit}분</td>
...
<table class="list_table" style="width:70%;margin-left: 10%;" id="tab2">
	...
	<thead>
		<tr>
			<th>적용여부</th> <th>제목</th> <th>시작 시간</th> <th>마감 시간</th> <th>할인율</th>
		</tr>
	</thead>
	...
	<tbody>	
		...
		<c:if test="${all.del != 1}">
			...
			<td>${all.title}</td> 
			<td>${all.shour}시 ${all.smin}분</td>
			<td>${all.timeremit}분</td>
			<td>${all.discount}</td>
		</tr>
		</c:forEach>
	</tbody>
...
<script type="text/javascript">
	$("#_regit").click(function () {
		...
		if(seq == null){
			...
			location.href="regcoupon.do?seq=" + seq + "&sort=3";	
		}
	});
	$("#tab2").hide();
	function sel(num) {
